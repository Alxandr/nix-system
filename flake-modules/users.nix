...
in
{
  ...
  config =
      ...
      getUserNixosModuleList = userName: userModule: {
        ...
        value =
          ...
          let
            ...
            generatedModule = {
              config = {
                nix.settings.trusted-users = lib.mkIf userDefModule.config.trusted [ userName ];
                programs._1password-gui.polkitPolicyOwners = lib.mkIf userDefModule.config._1password [ userName ];
    ...
    in
    {
      flake.users = lib.listToAttrs userModules;
    };
}
